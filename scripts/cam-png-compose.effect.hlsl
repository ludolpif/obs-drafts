...
float3 decode_gamma(float3 color, float exponent, float shift)
{
    ...
    float3 unshifted_exponent = float3(tmp, tmp, tmp);
    return pow(clamp(color, 0.0, 1.0), unshifted_exponent);
}

float3 encode_gamma(float3 color, float exponent)
{
    float tmp = 1.0/exponent;
    float3 inv_exponent = float3(tmp, tmp, tmp);
    return pow(clamp(color, 0.0, 1.0), inv_exponent);
}
...
float4 pixel_shader_halftone(pixel_data pixel) : TARGET
{
    ...
    float3 result = float3( luminance+perturbation, luminance+perturbation, luminance+perturbation );
