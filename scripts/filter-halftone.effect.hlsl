...
{
    float exp = exponent - shift;
    return pow(clamp(color, 0.0, 1.0), float3(exp,exp,exp));
}

float3 encode_gamma(float3 color, float exponent)
{
    float exp = 1.0/exponent;
    return pow(clamp(color, 0.0, 1.0), float3(exp,exp,exp));
}
...
float4 pixel_shader_halftone(pixel_data pixel) : TARGET
{
    ...
    float tmp = luminance + perturbation;
    float3 result = float3(tmp,tmp,tmp);
